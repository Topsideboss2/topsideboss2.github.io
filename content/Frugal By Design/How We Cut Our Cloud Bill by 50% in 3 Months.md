...
They are not. They are everyone's problem — and once we started treating them that way, the results were dramatic.
...
Cost visibility was low. Accountability was lower. Nobody had a clear picture of what anything cost, which meant nobody had a clear incentive to optimise it.
...
Every time a node scheduled a new pod, it potentially pulled a fresh copy of the image from the public registry. This traffic left our VPC, hit the NAT Gateway, and we were billed for every byte.

The fix was **AWS ECR Pull-Through Cache**.

ECR Pull-Through Cache allows you to configure ECR as a transparent caching proxy for upstream public registries. The first pull of an image fetches it from the upstream registry and stores it in your private ECR. Every subsequent pull comes from ECR directly — inside the AWS network, with no NAT Gateway traversal and no per-byte charge.

The setup change was minimal. The cost impact was not. NAT Gateway data transfer costs dropped substantially once the cache warmed up and became the primary source for image pulls across the cluster.
...
The problem was the architecture around those objects, not the objects themselves. We had enabled object versioning without lifecycle rules, which meant every version of every artifact was being retained indefinitely in standard storage. Months of build output, spanning dozens of pipelines, accumulating at standard S3 pricing.

We fixed this in two steps:

1. **Lifecycle rules**: Objects were automatically transitioned from S3 Standard to S3 Standard-IA (infrequent access) after 30 days, and to S3 Glacier after 90 days. Artifacts that genuinely needed long-term retention were now stored at a fraction of the original cost.
2. **Retention policy**: Old versions beyond a defined retention window were expired and deleted automatically.

This alone cut a meaningful portion of our S3 spend without changing anything about how the pipeline worked from the consumer's perspective.

### RDS: Right-Sizing Instances and Upgrading Storage

At the database layer, we found two separate opportunities.
...
Second — and this one is easy to miss — we migrated all RDS instances from **gp2** to **gp3** EBS volumes. gp3 is the current generation of general-purpose SSD storage in AWS, and it is simply cheaper than gp2: **approximately 20% less per GB**. It also offers better baseline performance and allows you to independently provision IOPS and throughput without being tied to storage size as gp2 requires. There is no performance trade-off for most database workloads — the upgrade is straightforward and the savings are immediate.
...
**AWS Savings Plans** offer a discount of up to 66% on EC2 compute costs in exchange for a one- or three-year commitment to a consistent level of spend. The key is knowing what you are committing to — purchasing Savings Plans before right-sizing locks in spending that may be higher than necessary.
...
This created a closed loop: our commitments matched our actual usage, our actual usage stayed within our committed instance families, and discounts applied automatically without manual intervention.

---

## 6. Spot Instances for Interruptible Workloads

Not every workload needs guaranteed compute. Some workloads are naturally fault-tolerant: they retry on failure, process jobs idempotently, or can simply be rescheduled without user impact. For these, **EC2 Spot Instances** offer discounts of 60–90% compared to on-demand pricing in exchange for the possibility of interruption.

We identified two clear candidates: **Airbyte** data sync jobs and **GitLab CI/CD runners**. Both are stateless, retriable, and have no strict latency requirements that would make an interruption consequential.

Using Kubernetes **Taints and Tolerations**, we configured Karpenter to provision Spot nodes specifically for these workloads. The taint marks a node as Spot-only; only pods with the matching toleration are scheduled onto it. This ensures that production-sensitive services never land on interruptible nodes, while the workloads that can tolerate interruption benefit from dramatically lower compute costs.
...
The reason we have not migrated yet is straightforward: it requires work. Some dependencies, native libraries, and base images need to be rebuilt or verified for ARM compatibility. This is not a weekend task for a complex platform — there are architectural differences that require careful testing. But it is firmly on the roadmap, and the long-term payoff will be significant.
...
- Designating **Cost Champions** — engineers within each team who take ownership of their product's cloud spend
...
Fifty percent in three months sounds like a dramatic headline, but it was the result of a structured process: understand what you are spending, attribute it accurately, eliminate waste, optimise what remains, and commit to what you know.

The individual techniques — Karpenter, KEDA, ECR Pull-Through Cache, gp3 upgrades, Savings Plans, Spot instances — are all well-documented and available to any AWS team. What makes the difference is the willingness to treat cost as a first-class engineering concern, and to give engineers the visibility and accountability to act on it.
